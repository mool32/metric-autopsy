...
**Relation to existing tooling.** Established single-cell QC packages (e.g. scran, scater, and probabilistic cell-quality filters such as miQC) score and filter *cells* on quality; batch-integration methods correct *cell embeddings*. None of these ask whether a *downstream scalar metric* moves for a QC reason — that gap is what this tool targets. The confounds it encodes are well known individually: the depth-dependence of correlation and information metrics under dropout, library-size normalization artifacts, and interaction confounds of the Simpson's-paradox kind that pooling hides. The contribution here is not a new statistic but a runnable, metric-agnostic red-teaming sequence that forces those known checks to be applied, in order, before a metric is believed.
...
**How it is computed.** When you pass a second `data` object, the engine re-runs GATE 1 (QC parity) and GATE 2 (n_genes matching) on it; replication is recorded as PASS only if the matched effect passes GATE 2 on the independent data. In v0.1 this gate is a documented procedure demonstrated in `examples/`; without a second dataset it returns SKIP.
